...
directoryContent* getDirectoryContents(int* noOfitems,int inode_number)
{
	...
	if(isAllocated & isDirectory)
	{
		...
		*noOfitems = sizeToRead/sizeof(directoryitem_t);	
		list = malloc(sizeof(directoryContent) * 2);
		//Check the file size
		short isLargeFile = ((directoryInode.flags & (1 << 12)) >> 12); 
		
				
			DEBUG_LOG("\n\t %d is a small directory",inode_number);
			int bytesread =0;
			directoryitem_t dir;

			for(i=0;i<len(directoryInode.addr) && sizeToRead!=0 && directoryInode.addr[i] !=0;i++)
			{
				totalbytesReadPerBlock =0;
				//Go to the block and read the directory contents
				lseek(fd,BLOCK_POSITION(directoryInode.addr[i]),SEEK_SET);
				//Read till the end of block or till the end of file
...
void deleteDirectoryEntry(int parent_inode_number,int inode_number)
{
	...
	//Find Data block to delete
			...
			int bytesread =0;
			for(i=0;i<len(parent_inode.addr) && sizeToRead && parent_inode.addr[i]!=0;i++)
			{
				totalbytesReadPerBlock = 0;
				//Go to the block and read the directory contents
				lseek(fd,BLOCK_POSITION(parent_inode.addr[i]),SEEK_SET);
...
void deleteDir(int inode_number)
{
	...
	{	
		...
		{
			parent_directoryInode = directoryContents[i].inode;
		}
...
void cpin(char* args)
{
	...
	//copying filepath to a temp variable
	v6filePath =  malloc(sizeof(v6fileName));
	strcpy(v6filePath,v6fileName);
	
	//Gets the last part of the filepath. (seperated by '/')
	targetFileName = getfileName(v6filePath);

	int parent_inode_number = 1;
	int inode_number = 0;

	// Update parent inode based on the input v6 file path
	if(v6fileName[0] =='/')
	...
	while(bytesRead = read(efd,&readbuffer,sizeof(readbuffer)))
	{
		...
		if(bytesWrittenPerBlock == 0)
		{
			...
			if(blockNumber)
				addDataBlockToInode(inode_number,blockNumber); // Adding the block to addr[]
			else
				return;
  		}
		...
		// To reset the offset and check if the current block is full
